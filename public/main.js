...
const executeManualMove = (direction) => {
  ...
  if (isEnd(nextState)) {
    setTimeout(function () {
      // Include the current level name in the alert
      window.alert(`Completed level: ${DATA.current}`);
...
const resetLevel = () => {
  ...
  loadLevel(DATA.current)
  console.log(`Level ${DATA.current} has been reset to its initial state.`)
}
